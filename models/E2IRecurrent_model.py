...
class E2IrecurrentModel(BaseModel):
    def __init__(self, opt):
        ...
        self.visual_names = ['evs', 'images']
        if self.isTrain:
            self.visual_names.append('gt')

        self.metric_names = ['psnr', 'ssim']
        self.model_names = ['Encoder_E', 'Decoder']
        ...
        self.netEncoder_E = networks.define_Encoder(Etype=weights, in_channels = opt.num_bins, train = True, gpu_ids=self.gpu_ids)
        self.netDecoder  = networks.define_Decoder(Dtype=weights, out_channels = 3, train = True, gpu_ids=self.gpu_ids)
    ...
    def forward_each(self, evs):
        feat, self.state = self.netEncoder_E(evs, self.state)
    ...
    def optimize_parameters(self):
        self.state = None
        imgs, gts = [], []
        for evs, gt in zip(self.evs, self.gt):
            self.images = self.forward_each(evs.to(self.device).float())
